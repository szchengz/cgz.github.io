...
/**
 * Created by cgz on 2019-05-28 17:11
 * 描述：
 */
public class LuozhuangshenshaHehun {
    ...
    public String shensha(Calendar cal) throws ParseException {
        ...
        //取表格中的中国农历月份
        // 取月份，其实原来表格不很合理，因为没有考虑闰月和节气问题
        // 正规算八字按节气算，即使闰月也不怕——大师♂罗莊
        //建议按照文章中根据五行纳音或者属相来处理某些神煞
        // 这里要帮大家解决通用问题，我就把四柱和四柱五行纳音都取出来
        // 然后大家根据substring 取出五行和属相
        ...
        String soundtime = getnumsix(ganzitime);
        //表格有14格 //其中属相从数组1-12，顺序就是子1，丑2这样
        // 使用者注意：本代码只是简单取表，没有考虑男女和多月分以及五行纳音
        int number = lunar.getYearindex() + 1;
        StringBuffer result = new StringBuffer();
        for (int i = 0; i < 14; i++) {
            if (month.equalsIgnoreCase(shensha1[number][ i])) {
                result.append("此人犯" + shensha1[0][ i]); result.append("\n");
            }
                if (month.equalsIgnoreCase(shensha2[number][i])) {
                    result.append("此人犯" + shensha2[0][ i]); result.append("\n");
                } }
    ...
    public int getnumber(String year) throws NumberFormatException {
        int
            yearnumber = Short.parseShort(year); return yearnumber;
    }
